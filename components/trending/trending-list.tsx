    ...
    <div className="mx-auto max-w-2xl py-12 px-6 sm:px-0">
      <div className="flex flex-col">
        {articles.map((article, index) => (
          <Link key={article.id} href={`/trending/${article.id}`} className="block group">
            <motion.article
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: index * 0.05, duration: 0.5 }}
              className="relative border-b border-primary/10 py-10 last:border-0"
            >
              <div className="space-y-4">
                {/* Meta information */}
                <div className="flex items-center gap-2 text-xs font-bold tracking-widest uppercase text-muted-foreground/40">
                  <span className="text-primary/60">{article.meta.topic}</span>
                  <span className="h-0.5 w-0.5 rounded-full bg-primary/10" />
              ...
              <div className="absolute -inset-x-4 -inset-y-2 -z-10 rounded-2xl bg-primary/5 opacity-0 transition-opacity group-hover:opacity-100" />
            </motion.article>
          </Link>
        ))}
